...
## 8. V1.1.1 真机反馈修复（iPhone / iPad 实机截图）

上线到 `solar-system-atlas.pages.dev` 之后在真机上暴露出来的问题，逐条定位并修掉。
每条都写了根因，方便下次不再犯。

| 真机现象 | 根因 | 修复 |
| --- | --- | --- |
| 手机打开"完全不知道是什么、没有侧视图、轨道看不见" | 我把手机竖屏默认落点改成了"俯视实时太阳系"（自以为是的设计）；`atlasShot` 的宽高比下限 0.6 在 0.46 的竖屏上会把左右两端切掉 | 撤销该默认落点，手机回到与桌面 / iPad 相同的侧视图；宽高比下限降到 0.42，整段跨度完整落进屏幕 |
| 手机 / iPad "轨道 UI 完全看不见" | `satelliteVisibility` 门限（24~96px）是在桌面 1600×900 上调的；竖屏可见高度更大、画布更矮，木星系统盘只剩 13~21px，全部掉到门限以下 | 新增 `lodGain()`：桌面 ×1 / 平板 ×2 / 手机 ×3.2，把 LOD 归一到桌面观感（桌面数值不变） |
| 聚焦之后"没法转动视角" | 聚焦状态每帧都把 yaw/pitch 拉回聚焦构图；桌面靠 `drag.current.moved` 盖住这条，触摸路径没有对应闩锁 → 手指一松就弹回 | 新增 `touchManualOrbit` 闩锁（只由触摸置位，换焦点 / 返回 / 切模式时复位）；桌面路径一行未改 |
| 手指"往左拖画面往右" | 触摸旋转的 yaw 符号写反了（`-=` 应为 `+=`），与桌面右键方向相反 | 与桌面公式对齐；实测手指右滑 120px → yaw −0.352（桌面右键 200px → −0.64，系数一致） |
| iPad 切"实时位置"后"聚焦点不在太阳" | `realPositionShot` 被我错误地套上了 `focusTarget` 的"主体靠左"偏移（桌面也受影响，是回归） | 目标点回到原点（太阳居中），桌面数值同时恢复到 x=0 的冻结基准 |
| iPad "右上角 UI 重叠" | iPad 同时渲染了完整顶栏和移动条（主页 / 菜单 / EN），两套导航压在一起；遥测 HUD 又与顶栏同高 | 移动条 / 底部动作条 / 菜单抽屉改为只给手机；触屏设备隐藏遥测 HUD（`?debug=1` 的 `__atlasPerf()` 仍在） |
| iPad "信息页被挡住 / 时间轴压到档案底下" | 面板宽度写了两套值（变量 37vw、实际 width 38vw），时间轴的让位量用变量算，于是对不上 | 面板宽度只由 `--panel-w` 决定；实测面板起于 870、时间轴止于 846，不再重叠 |
| 手机聚焦"信息 UI 被遮挡、什么都看不见" | 详情抽屉默认停在 collapsed（只露标题），球面全息标注又铺了十几条互相压字 | 抽屉默认落到 half（正文直接可见）；手机全息标注限制为 Top 3，抬头与右侧层结尺在手机隐藏 / 夹进可视区 |
| "整体 UI 在打架、位置不对" | 引用长廊在竖屏与品牌标题叠字（"145 收录对象"压 "ASTRONOMICAL VOICES"） | 竖屏与手机隐藏引用长廊（横屏平板保留） |
| "贴图很糊 / 背景像噪点" | 降级阶梯最后两步会把渲染分辨率压到 1.0 以下（1.35×0.7=0.95），整幅发虚；星点按颗数给，手机上单位像素密度比桌面高 1.7 倍 | DPR 加 1.0 地板（移动端永不低于 1）；星点按屏幕面积对齐桌面：平板 ×0.55、手机 ×0.3 |

修复后重测的桌面回归：`height 113.95 / target.x 101 / yaw 0 / pitch 0`、
`.timeline [34,579,1506,159]`、行星标签屏幕坐标逐字符相同、右键拖拽 200px → yaw −0.64。
实时位置模式的目标点也从被改坏的 x=148.54 恢复到 **x=0**。
